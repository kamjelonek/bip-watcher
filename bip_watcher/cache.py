...
from bip_watcher.utils import now_iso, sha1
...
def load_cache_v2():
    ...
    try:
        ...
        print(f"📦 Cache loaded: {len(urls)} URLs, {len(content)} content, {len(gseeds)} gmina seeds, {len(pf)} page_fprints, {len(dead)} dead")
...
def purge_old_cache(raw_cache: dict, urls_seen_set: set, content_seen: dict, gmina_seeds: dict, page_fprints: dict, dead_urls: dict):
    ...
    # Martwe strony TTL handling left as-is
