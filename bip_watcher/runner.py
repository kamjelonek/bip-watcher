...
from bip_watcher.cache import (
    load_cache_v2,
    save_cache_v2,
    purge_old_cache,
)

from bip_watcher.diagnostics.summary import (
    save_diag,
    write_summary,
...
async def main():
    ...
    print(
        f"🧩 SHARD {shard_index}/{shard_total} -> {len(rows)}/{len(rows_all)} gmin",
        flush=True,
    )
    ...
    timeout_quick = aiohttp.ClientTimeout(
        total=None,
        sock_connect=12,
        sock_read=35,
    )
    ...
    async with aiohttp.ClientSession(
        connector=conn_default, timeout=timeout_quick
    ) as s_default, aiohttp.ClientSession(
        connector=conn_ipv4, timeout=timeout_quick
    ) as s_ipv4, aiohttp.ClientSession(
        connector=conn_crawl, timeout=timeout_quick
    ) as s_crawl:
